...
TOKEN = "8236349814:AAGpaze8Z0f93pcRrWG55A4ckeVVfvpZ1xA"
OWNER_IDS = [938266085, 1071484746]  # два администратора
...
# Пересылка сообщений владельцам
            ...
            try:
                if message.content_type == 'text':
                    ...
                    try:
                        ...
                        bot.send_document(owner_id, file_id, caption=f"Фото от {sender}")
                else:
...
def handle_text(message):
    ...
    forward_to_owner(message)  # пересылаем владельцам
...
def handle_file(message):
    ...
    if has_keyword:
        ...
        # Пересылаем файл всем администраторам
        file_id = message.document.file_id
        for owner_id in OWNER_IDS:
